...
private fun EstimationList(estimatedResult: EstimatedResult) {
    Column(
        ...
        estimatedResult.estimations.mapIndexed { index, estimation ->
            RowResult(
                estimation = estimation.map { it.number },
...
private fun RowResult(
    ...
    val borderColor =
        if (estimation.filter { es -> es in winnerList }.size > 2) Color.Green.copy(alpha = 0.5f) else MaterialTheme.colorScheme.primaryContainer
