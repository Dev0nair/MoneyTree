...
import com.ismaelgr.domain.manager.statiticWithUsuals
...
class CalculateSetsToWinUseCase @Inject constructor(
    ...
    operator fun invoke(input: Data.Input) = flow<Data.Output> {
        ...
        if (!dateResults?.numberList.isNullOrEmpty()) {
            val pnUntilDate: List<NumberData> =
                punctuationManager.generatePnOfDate(input.date, false).map { dd -> dd.numberData }
            val pdUntilDate: List<PDResult> = punctuationManager.generatePdOfDate(input.date, true)
            val usualPNs = statisticManager.getTopPns(input.date, topCount = Int.MAX_VALUE)

            val statistics = statisticManager
                .generateStatistics(date = input.date, pds = pdUntilDate, pns = pnUntilDate)
                .toSet()
                .toList()
                .statiticWithUsuals(usualPNs)
            ...
            while (setsGenerated.size < input.maxTries && checkPrize() < input.profit) {
                val newSet =
                    statisticManager.newGenerateSet(statistics.filter { st -> st !in setsGenerated.flatten() })
            ...
            emit(
                Data.Output(
                    date = input.date,
                    numSetsNeeded = setsGenerated.size,
                    totalWin = checkPrize()
                )
            )
        }
