...
class GenerateEstimationOfDateUseCase @Inject constructor(
    ...
    operator fun invoke(date: String): Flow<EstimatedResult> = flow {
        ...
        val pnUntilDate: List<NumberData> =
            punctuationManager.generatePnOfDate(date, false).map { dd -> dd.numberData }
        val pdUntilDate: List<PDResult> = punctuationManager.generatePdOfDate(date, false)
        val statistics =
            statisticManager.generateStatistics(date = date, pds = pdUntilDate, pns = pnUntilDate)
                .toSet().toList()
        ...
        val winnerAnalysis: List<Summary> = dateResults?.numberList?.mapNotNull { winner ->
            ...
            Summary(
                number = d.number,
                pn = d.punctuation,
                pd = statisticManager.chooseNearestPD(listPDs = pdUntilDate, d.punctuation, 1)
                    .firstOrNull()
            )
        }
            ?: emptyList()
