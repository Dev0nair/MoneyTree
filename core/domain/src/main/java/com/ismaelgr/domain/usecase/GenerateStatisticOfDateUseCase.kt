    ...
    operator fun invoke(numDays: Int): Flow<Unit> = flow {
        ((-1 * numDays)..0).forEach { i ->
            ...
            iRepository.getResult(currentDate)?.numberList?.let { winnerList ->
                ...
                val pnList: List<DateData> = punctuationManager.generatePnOfDate(currentDate, true)
                    .filter { dd -> dd.numberData.number in winnerList }
                pnList.forEach(iRepository::addDateData)
                // De aquí solo cogemos los PD ganadores
                val pdList: List<PDResult> = punctuationManager.generatePdOfDate(currentDate, true)
                    .filter { pd -> pd.pn in pnList.map { pn -> pn.numberData.punctuation } }
    ...
    fun backgroundLoading(numDays: Int): Flow<Unit> = flow {
        ((-1 * numDays)..0).forEach { i ->
            ...
            GlobalScope.launch(Dispatchers.Default) {
                iRepository.getResult(currentDate)?.numberList?.let { winnerList ->
                    val pnList: List<DateData> =
                        punctuationManager.generatePnOfDate(currentDate, true)
                            .filter { dd -> dd.numberData.number in winnerList }
                    pnList.forEach(iRepository::addDateData)
                    val pdList: List<PDResult> =
                        punctuationManager.generatePdOfDate(currentDate, true)
                            .filter { pd -> pd.pn in pnList.map { pn -> pn.numberData.punctuation } }
                    pdList.forEach(iRepository::addPDResult)
                }
            }
